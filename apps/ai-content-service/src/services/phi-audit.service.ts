...
/**
 * Durable sink for `auditLog()` in this service. Same contract as
 * user-provider's, writing to this service's own `phi_access_log` — see the
 * table comment in db/schema.ts for why the trail is per-database. Writes one
 * append-only row per request that touched health data — successful reads and
 * writes, and denials.
 *
