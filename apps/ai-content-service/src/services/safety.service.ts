...
const PII_PATTERNS: Array<{ name: string; pattern: RegExp; replacement: string }> = [
  ...
  },
  {
    name: 'address',
    pattern:
      /\b\d{1,5}\s+\w+(\s+\w+)*\s+(street|st|avenue|ave|boulevard|blvd|drive|dr|road|rd|lane|ln|court|ct|way|place|pl)\b/gi,
...
export class SafetyService {
  ...
  stripPii(
  ...
  ): { cleaned: string; piiFound: string[] } {
    let cleaned = text;
    ...
    if (context?.dateOfBirth) {
      ...
      cleaned = cleaned.replace(
        new RegExp(context.dateOfBirth.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'),
        `[AGE_RANGE: ${ageRange}]`,
      );
  ...
  async processInput(
    ...
    if (blocked) {
      logger.warn(
        { safetyLogId: safetyLog.id, patterns: injection.patterns },
