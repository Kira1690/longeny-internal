...
async function handleBookingCreated(event: EventEnvelope<BookingCreatedPayload>) {
  ...
  const [order] = await db
    ...
    .values({
      ...
      tax: '0',
      platform_fee: Number.parseFloat((amount * 0.1).toFixed(2)).toString(),
...
async function handleBookingCancelled(event: EventEnvelope<BookingCancelledPayload>) {
  ...
  const [order] = await db.select().from(orders).where(eq(orders.booking_id, bookingId)).limit(1);
...
async function handleGdprErasure(event: EventEnvelope<GdprErasurePayload>) {
  ...
  try {
    ...
    await db
      ...
      .where(and(eq(subscriptions.user_id, userId), not(eq(subscriptions.status, 'cancelled'))));
