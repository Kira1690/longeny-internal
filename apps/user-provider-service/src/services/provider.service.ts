...
export class ProviderService {
  ...
  async updateProfile(
    ...
    if (data.serviceAreaRadiusMiles !== undefined)
      updateData.service_area_radius_miles = data.serviceAreaRadiusMiles;
    ...
    if (data.cancellationPolicy !== undefined)
      updateData.cancellation_policy = data.cancellationPolicy;
    if (data.cancellationHours !== undefined)
      updateData.cancellation_hours = data.cancellationHours;
  ...
  async submitVerification(
    ...
    logger.info(
      { providerId: provider.id, verificationId: verification.id },
  ...
  async removeAvailabilityOverride(authId: string, overrideId: string) {
    ...
    const [override] = await db
      ...
      .where(
        and(
          eq(availability_overrides.id, overrideId),
          eq(availability_overrides.provider_id, provider.id),
        ),
      )
      .limit(1);
  ...
  async createProgram(
    ...
    const [program] = await db
      ...
      .values({
        ...
        session_count: data.sessionsPerWeek
          ? (data.durationWeeks || 1) * data.sessionsPerWeek
          : undefined,
  ...
  async listProviders(filters: {
    ...
    if (filters.offersVirtual !== undefined)
      conditions.push(eq(providers.offers_virtual, filters.offersVirtual));
    if (filters.offersInPerson !== undefined)
      conditions.push(eq(providers.offers_in_person, filters.offersInPerson));
    ...
    const providersList = await db
      ...
      .from(providers)
      .where(whereClause)
      .orderBy(desc(providers.rating_avg))
      .limit(limit)
      .offset(offset);
  ...
  async getPublicAvailability(providerId: string) {
    ...
    const rules = await db
      ...
      .from(availability_rules)
      .where(
        and(eq(availability_rules.provider_id, providerId), eq(availability_rules.is_active, true)),
      )
      .orderBy(asc(availability_rules.day_of_week), asc(availability_rules.start_time));
  ...
  async getProviderStats(authId: string) {
    ...
    const activePrograms = await db.$count(
      programs,
      and(eq(programs.provider_id, provider.id), eq(programs.status, 'active')),
    );
    const totalProducts = await db.$count(products, eq(products.provider_id, provider.id));
    const activeProducts = await db.$count(
      products,
      and(eq(products.provider_id, provider.id), eq(products.status, 'active')),
    );
    const reviewCount = await db.$count(
      reviews,
      and(eq(reviews.target_type, 'PROVIDER'), eq(reviews.target_id, provider.id)),
    );
